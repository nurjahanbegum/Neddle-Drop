...
{
    public partial class frmRptAdmResult : Form
    {
        ...
        public frmRptAdmResult(ref Infragistics.Win.UltraWinTabControl.UltraTabControl utab, Common.FormEntry.frmMaster fm)
        {
            InitializeComponent();
            uTab = utab;
            FM = fm;
        ...
        private void prcShowReport()
        {
            try
            {
                ...
                FormCaption = "Report :: Admission Result...";
                ...
                else if (optCriteria.Value.ToString().ToUpper() == "Merit".ToUpper())
                {
                    Type = optCriteria.Value.ToString();
                }
                if (optCriteria.Value.ToString().ToUpper() == "Wait".ToUpper())
                {
                    Type = optCriteria.Value.ToString();
                }
                ...
                rptQuery = "Exec gtrhris.dbo.rptAdmResultSheet " + Common.Classes.clsMain.intComId + ",'" + Type + "', 0, " + gridClass.ActiveRow.Cells["clsId"].Value + ", '" + gridSession.ActiveRow.Cells["sesn"].Value + "', '" + clsProc.GTRDate(dtDateFrom.Value.ToString()) + "', '" + clsProc.GTRDate(dtDateTo.Value.ToString()) + "', 0 ";
        ...
        {
            ReportPath = Common.Classes.clsMain.AppPath + @"\Campus\Reports\rptAdmResultSheet.rdlc";
        ...
        private void frmRptAdmResult_Load(object sender, EventArgs e)
        {
            try
            {
                prcLoadList();
        ...
        private void frmRptAdmResult_FormClosing(object sender, FormClosingEventArgs e)
        {
