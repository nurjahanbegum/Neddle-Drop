...
{
    public partial class frmSalDue : Form
    {
        ...
        public frmSalDue(ref Infragistics.Win.UltraWinTabControl.UltraTabControl utab, Common.FormEntry.frmMaster fm)
        {
            InitializeComponent();
            uTab = utab;
            FM = fm;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //Update data
                    sqlQuery = " Update tblSal_Due Set dtInput = '" + clsProc.GTRDate(dtInputDate.Value.ToString()) + "', amount = '" + clsProc.GTRValidateDouble(txtAmt.Value.ToString()) + "',remarks = '" + txtRemarks.Text.ToString() + "' Where DueID = " + Int32.Parse(txtId.Text.ToString());
                ...
                {
                    sqlQuery = "Select dbo.fncCheckData (" + Common.Classes.clsMain.intComId + ",'" + cboEmpID.Value.ToString() + "','" + clsProc.GTRDate(dtInputDate.Value.ToString()) + "','Salary Production')";
                    ChkData = clsCon.GTRCountingDataLarge(sqlQuery);

                    if (ChkData == 1)
                    {
                        MessageBox.Show("This employee's production amount already inputted. Please select another employee Id.");
                        return;
                    }
                    
                    //add new
                    sqlQuery = "Select Isnull(Max(DueID),0)+1 As NewId from tblSal_Due";
                    NewId = clsCon.GTRCountingData(sqlQuery);

                    //Insert data
                    sqlQuery = "Insert Into tblSal_Due ( aId,DueID, empid, dtInput,amount, Remarks,LUserId,comid ,PcName) "
        ...
        private void gridList_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
        {
            try
            {
                ...
                gridList.DisplayLayout.Bands[0].Columns["DueID"].Hidden = true;  //DueID
        ...
        private void prcLoadList()
        {
            ...
            try
            {
                string sqlQuery = "Exec prcGetSalDue 0," + Common.Classes.clsMain.intComId + ",0,'" + clsProc.GTRDate(dtDate.Value.ToString()) + "'";
        ...
        private void frmSalDue_Load(object sender, EventArgs e)
        {
            try
            {
                prcClearData();
        ...
        private void frmSalDue_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        {
            if (MessageBox.Show("Do you want to Delete Salary Allowance  [" + txtName.Text + "] this Employee" , "", System.Windows.Forms.MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.No)
            {
                return;
            }
            ...
            {
                string sqlQuery = "";
                sqlQuery = "Delete from tblSal_Due Where DueID = " + Int32.Parse(txtId.Text);
                arQuery.Add(sqlQuery);
        ...
        private void prcDisplayDetails(string strParam)
        {
            ...
            try
            {
                string sqlQuery = "Exec prcGetSalDue " + Int32.Parse(strParam) + "," + Common.Classes.clsMain.intComId + " ,1,'" + clsProc.GTRDate(dtDate.Value.ToString()) + "'";
                ...
                if (dsDetails.Tables["tblSalDue"].Rows.Count > 0)
                {
                    ...
                    this.txtId.Text = dr["DueID"].ToString();
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string sqlQuery = "Exec prcGetSalDue 0," + Common.Classes.clsMain.intComId + ",1,'" + clsProc.GTRDate(dtDate.Value.ToString()) + "'";
                clsCon.GTRFillDatasetWithSQLCommand(ref dsList, sqlQuery);
                dsList.Tables[0].TableName = "tblEmployee";
